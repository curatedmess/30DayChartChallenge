...
df %>% 
  ...
  geom_image(aes(x = 1.25, y = 0.15, image = image), size = 0.08, by = "height", asp = 2.6, hjust = 0) +
  ...
  guides(size = guide_legend(title.position="top", title.hjust = 0.5, title = "Number of Hybrids")) +
  theme_void() +
  theme(plot.title = element_markdown(family = font_t, size = 39, hjust = 0, face = "bold", color = "#000000"),
        ...
        legend.position = "top",
        legend.text = element_text(size = 7, family = font, color ="#000000"),
        legend.title = element_text(size = 8, family = font, color ="#000000"),
